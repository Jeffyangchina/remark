...
def Weights(shape):
    w = tf.truncated_normal(shape, stddev=0.01, dtype=tf.float32)
...
out_con1 = maxpool(h_conv1)  # output size 16x16x32
# define conv2 layer2
W_conv2 = Weights([5, 5, 32, 64])
b_conv2 = Biases([64])
h_conv2 = tf.nn.relu(conv2d(out_con1, W_conv2) + b_conv2)
out_con2 = maxpool(h_conv2)  # output size 10,8x8x64

# define function layer1
W_func1 = Weights([8 * 8 * 64, 1024])  # insize,outsize
tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(0.0003)(W_func1))
b_func1 = Biases([1024])  # outsize
out_conv2_flat = tf.reshape(out_con2, [-1, 8 * 8 * 64])  # [n_sample,6,6,64]>>[n_sample,6*6*64]
...
#这个函数的输入logits需要时未经tf.nn.softmax的，
    ...
    cross_entropy_loss=-tf.reduce_sum(tf.cast(ys,tf.float32)*tf.log(tf.cast(prediction,tf.float32)),reduction_indices=[1])
...
# train
try:
    dr = cf10.Cifar10DataReader(cifar_folder=finame)


except Exception as s:
    print('wrong0',s)
with sess.as_default():
    saver.restore(sess, 'train/cifar10.ckpt')
    ...
    for i in range(1000):  #
        try:
            img, label = dr.next_train_data(batch_size=batch_size)#input label is one-hot
        ...
        try:
            ...
            sess.run(train_step, feed_dict={xs:img, ys:label, keep_prob: 0.6})  # x_data,y_data为数据集
        except Exception as s:
            print('wrong2', s)
        if i % 10 == 0:
            try:
                train_accuracy = accuracy.eval(feed_dict={ xs:img, ys:label, keep_prob: 1})
    ...
    # coord.join(threads)
    print('finished')
    save_path = saver.save(sess, 'train/cifar10.ckpt')
