...
class Cifar10DataReader():
    ...
    def _decode(self, datum, onehot):
        ...
        if onehot:
            for d, l in datum:
                rdata.append(np.reshape(np.reshape(d, [3, 1024]).T, [32, 32, 3])* (1. / 255) - 0.5)
                hot = np.zeros(10)
                hot[int(l)] = 1
                rlabel.append(hot)
        else:
...
#     dr = Cifar10DataReader(cifar_folder=finame)
