...
import string
from sklearn.preprocessing import MultiLabelBinarizer  # 2018
...
def createLabelDict(hierarchy):
    ...
    for line in open(hierarchy):
        ...
        # line = [x.strip() for x in line]#2018
        line = rm_null(line)
        item = line[0]
...
def training(X, Y, X_test, pca='kpca', regressor='ridge', dim=50):
    ...
    # print('train:',X_test.shape)#2018
    # print('train2:',P.shape)#2018
...
def CSSAG(L, y_pred, prop='and'):
    """ CSSAG param L:num of items, y_pred:set of items,prop: DAG type "and" or "or". """
    psi = {}
    for item in labelDict:
        psi[(item,)] = 0
    psi[('ROOT',)] = 1
    ...
    # sorted_snv = sorted(list(graph.items()), cmp=lambda x,y:cmp(x['snv'], y['snv']), key=operator.itemgetter(1))
    sorted_snv = sorted(list(graph.items()), key=lambda x: x[1]['snv'])  # 排序
            ...
            if cond == True:
                ...
                for S_ in unassigned_nodes:
                    ...
                    for node in list(set(S_).difference(set(S))):
                        while (
                        node,) in graph: node += '_'  # ('_',) # Replication may create two nodes with same name, hence this
                        ...
                        sorted_snv = [(x, y) for x, y in sorted_snv if y['snv'] <= graph[node]['snv']] + [
                            (node, graph[node])] + [(x, y) for x, y in sorted_snv if y['snv'] > graph[node]['snv']]
                        psi[node] = 0
...
def prepareDataset(trainfile, trainlabels, testfile, testlabels):
    ...
    # print('0:',type(X),type(y_train))#scipy.csr_matrix,np.array
    ...
    print('xxx:',len(Y_test),len(Y_test[0]))
    return X, Y, X_test, Y_test


def pres_rec_f1(Y_true, Y_preds_list, mb):
    ...
    for n in [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]:
        Y_true = mb.transform(Y_true)

        Y_plist = mb.transform(Y_preds_list[n])  # Y_preds_list[n]=100*9
        if i < 2:
            print('0:', Y_preds_list[n][0])
            print('1:', len(Y_true[0]), Y_true[0])
            print('2:', len(Y_plist[0]), Y_plist[0])
            i += 1
        p, r, f, s = precision_recall_fscore_support(Y_true, Y_plist, average='samples')
        pres_sample.append(p)
        rec_sample.append(r)
        f1_sample.append(f)
        p, r, f, s = precision_recall_fscore_support(Y_true, Y_plist, average='micro')
        pres_micro.append(p)
        rec_micro.append(r)
        f1_micro.append(f)
        p, r, f, s = precision_recall_fscore_support(Y_true, Y_plist, average='macro')
        pres_macro.append(p)
        rec_macro.append(r)
        f1_macro.append(f)
    data_reg = {}
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-i', '--hierarchy', dest='hierarchy',
                      help='the file with the label hierarchy information: Each line in the file is <nodeID> <childNodeID1> <childNodeID2>... <childNodeIDn>',
                      type='str')
    ...
    parser.add_option('-x', '--test-labels', dest='testlabels',
                      help='the test labels file: comma separated labels per line', type='str')
    parser.add_option('-y', '--train-labels', dest='trainlabels',
                      help='the train labels file: comma separated labels per line', type='str')
    ...
    parser.add_option('-r', '--regressor', dest='regressor', type='str',
                      help='type of regressor to use: "ridge" or "svr"')
    parser.add_option('-p', '--pca', dest='pca', type='str',
                      help='type of pca: "pca"/"kpca"(kernel PCA)/"nopca"(a regressor is trained on each label)')
    ...
    tree = options.tree if options.tree != None else 'and'
    ...
    nc = list(range(NumClass))
    # Train and get prediction
    print('training model.......pca: ', pca, 'regressor: ', regressor, 'dim: ', dim, 'tree: ', tree)
    PCA, regressors, Y_pred = training(X, Y, X_test, pca, regressor,
                                       dim)  # simpleTraining(X, Y, X_test, pca, regressor, dim)
    ...
    for row in Y_pred:
        ...
        for i in range(len(row)):
            y[i] = row[i]
        sorted_y = sorted(list(y.items()), key=operator.itemgetter(1), reverse=True)
        for n in [5, 10]:
        # for n in [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]:
            Y_preds_list[n].append([x for x, y in sorted_y[:n]])  # 取出最大标签序号,共取前50
    ...
    for row in Y_test:
        ...
        Y_true.append(y)
    print('y_t:',Y_true[:3])
    # Mb = MultiLabelBinarizer().fit([nc])
    for rx in Y_pred:
        y = []
        for i in range(len(rx)):
            if rx[i] == 1: y.append(i)
        Y_pre.append(y)
    print('y_p:', Y_pre[:3])
    ...
    Y_preds_list = {5: [],10:[]}
    rex=[]
    # Y_pred=[[22,377],[1084,1104]]
    # Y_preds_list = {5: [], 10: [], 15: [], 20: [], 25: [], 30: [], 35: [], 40: [], 45: [], 50: []}
    for n in [5]:
        idx = 1
        print('at idx: ', idx, 'for n: ', n,'Y-pred:',len(Y_pred))
    ...
    print('0y:',len(rex),rex)
    # data_cssag = pres_rec_f1(Y_true, Y_preds_list, Mb)
